...
export default function Home(){
 ...
 return <main className={"tour "+(inside?"inside ":"")+(step===0?"arrival ":"")+(ended?"ending":"")}>
  ...
  <section className="story-dock" aria-label="Journey navigation">
   ...
   <div className="navigation-actions">{!free&&step>0&&<button className="back-button" disabled={busy} onClick={()=>navigate(step-1)} aria-label="Previous scene"><ArrowLeft size={19}/></button>}<button className="next-button" disabled={busy&&!free} onClick={()=>free?toggleFree():navigate(ended?0:step+1)} aria-label={free?"Return to journey":ended?"Replay journey":step===0?"Start the journey":"Next"}><span>{free?"Return to journey":ended?"Replay journey":step===0?"Start the journey":busy?"Moving into view…":step===8?"Continue to dispatch":"Next"}</span>{ended?<RotateCcw size={25}/>:<ArrowRight size={29}/>}</button></div>
  </section>
