...
export default function Home(){
 ...
 const current=journey[step],inside=step===5&&!free,ended=step===7;
 const ready=useCallback(()=>setBusy(false),[]);
 const navigate=useCallback((next:number)=>{setBusy(true);setStep(Math.max(0,Math.min(7,next)));setPlaying(true);setReset(n=>n+1);},[]);
 ...
 return <main className={"tour "+(inside?"inside ":"")+(step===0?"arrival ":"")+(ended?"ending":"")}>
  ...
  {inside&&<aside className="kiln-sequence" aria-label="Kiln AI use cases">{kilnCases.map((item,i)=><div key={item.title} className="case-marker active"><span>{String.fromCharCode(65+i)}</span><span>{item.title}</span><i/></div>)}<div className="decision-label">AI decision layer<small>Signals → recommendations → operator approval</small></div></aside>}
  <section className="story-dock" aria-label="Journey navigation">
   <div className="story-copy" key={free?"free":step} aria-live="polite"><span className="story-kicker">{free?"FREE EXPLORATION":current.kicker}</span><h2>{free?(freeStage===null?"Choose a stage to explore.":stages[freeStage].title):current.subtitle}</h2><p>{free?(freeStage===null?"Select a marker on the facility.":stages[freeStage].description):current.text}</p></div>
